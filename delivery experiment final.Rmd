...
---
# Introduction
As many pieces of research have shown, in this fast-paced society, people value the speed of delivery service. According to the Online Ordering Statistics(Resendes,2020), 31% of customers use delivery service at least twice a week. 33% of customers are willing to pay money in exchange for delivery speed.

The COVID-19 pandemic challenged our life in many aspects, for example, people are more likely to receive emails from third-party carriers informing them about the potential delay for their packages. But some industries grew fastly during this period, such as fresh delivery services. For this project, we would like to dive into the food delivery service industry. We’d like to take the standpoint of the third-party food delivery service platforms, such as Uber Eats, Chowbus, DoorDash, etc., which connect the restaurants and consumers. We want to analyze when everything holds constant, how the food delivery time interval will affect consumer’s willingness to place an order. 
...
For the questionnaire, first, we collected basic personal information, including age and gender, and we would like to know if the participant is a student or an employee for an organization and if he/she is working/studying from home. Second, we asked the participants' behavior of using food delivery service. For example, we would like to know the frequency of an individual orders food delivery, their preference of food category. We also decided to answer some questions about the previous service platform interaction recommend by K.Foreit and J.Foreit(2004) in their paper, the most frequently used food delivery service provider, and if the individual is a  prime member of any of the food delivery services. 
...
```
From the above summary we could observe that , there is a small differences on probability of ordering deliver services between users who favor chinese and not. In addition, users who favor ordering Chinese cuisine concern slightly more on delivery time range.
...
```
Based on above summary, we can conclude that average treatment effect varies by users' previous order frequency. Users had high previous order frequency tended to pay more attention on food delivery time range. Those users were 16.4% more likely to place an order with a short range of delivery time than with a relatively long range. Users had low previous order frequency, however, didn't really seem take range of delivery time into account. 
